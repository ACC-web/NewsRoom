...
class MultimediaGallery extends Component {
  ...
  filterImageGallary() {
    //filteredOptions
    var filter = this.state.galleryImages.filter(
      x =>
  ...
  componentDidMount() {
    ...
    const filtercategoriesOptions = this.props.filtercategories
      .filter(x => this.props.cats.map(v => v.toLowerCase().trim()).indexOf(x.node.category.toLowerCase().trim()) == -1)
      .map(item => {
        return {
    ...
    let allDataItems = props.galleryImages.map(item => {
      let categories = item.node.categories.map(cat => {
        return cat.category;
      });
      return {
    ...
    this.setState({
      ...
      galleryImages: allDataItems,
      galleryinfographic: galleryinfographic,
  ...
  openLightbox = (index, event, typeItem) => {
    ...
    this.setState({
      itemsLightbox: {
        ...
        items:
          typeItem == "photos" || typeItem == "infographic" || typeItem == "logos"
            ? this.state[typeItem]
  ...
  renderGallery() {
    ...
    //   cols=2;
    //  }
    return <Gallery items={this.state.pageOfItems} isLoading={this.state.isLoading} cols={cols} onClickItem={this.openLightbox} />;
  }
  render() {
    if (this.state.items) {
      const self = this;
        ...
        <Container>
          ...
          <div className={`media-gallery-wrapper ${this.state.tabIndex != 0 ? "fullgallery" : ""} ${this.state.tabIndex == 3 ? "infogallery" : ""}`}>
              ...
              <div className="visit-form-wrapper">
                                ...
                                <div className="flex-row">
                                  <CheckBoxContainer className="form-group">
                                    <input
                                      ...
                                      onClick={e => self.onhandlecheckboxChange(e, item)}
                                      id={item}
                                    />
                                      {/* REPLACED THIS WITH THE BELOW - label htmlFor="addToMedia">{item}</label>*/}
                ...
                <Tabs selectedIndex={this.state.tabIndex} onSelect={tabIndex => this.onSelectTab(tabIndex)}>
                  <TabList>
                    <Tab>
                      <div>
                        <span className="tab-text">Images</span>
                        <span>
                          <FaImages />
                        </span>
                      </div>
                    ...
                    <Tab>
                      <div>
                        <span className="tab-text">Logos</span>
                        <span>
                          <FaImage />
                        </span>
                      </div>
                    ...
                    <Tab>
                      <div>
                        <span className="tab-text">Infographics</span>
                        <span>
                          <FaUniversity />
                        </span>
                      </div>
            ...
            <Lightbox
              ...
              width={1600}
              isLightBoxLoading={this.state.isLightBoxLoading}
            />
          </div>
        </Container>
      );
